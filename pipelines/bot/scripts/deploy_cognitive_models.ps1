...
if (-not $name) {
    $name = Write-Host "? Base name for Cognitive Models"
}

if (-not $luisAuthoringRegion) {
    $luisAuthoringRegion = Write-Host "? LUIS Authoring Region (westus, westeurope, virginia (US gov-only) or australiaeast)"
}

if (-not $luisAuthoringKey) {
	Switch ($luisAuthoringRegion) {
		"westus" { 
			$luisAuthoringKey = Write-Host "? LUIS Authoring Key (found at https://luis.ai/user/settings)"
			Break
		}
        "virginia" {
			$luisAuthoringKey = Write-Host "? LUIS Authoring Key (found at https://luis.azure.us/user/settings)"
			Break
		}
		"westeurope" {
		    $luisAuthoringKey = Write-Host "? LUIS Authoring Key (found at https://eu.luis.ai/user/settings)"
			Break
		}
		"australiaeast" {
			$luisAuthoringKey = Write-Host "? LUIS Authoring Key (found at https://au.luis.ai/user/settings)"
			Break
		}
		default {
...
if (-not $luisAccountName) {
    $luisAccountName = Write-Host "? LUIS Service Name (existing service in Azure required)"
}
...
if (-not $resourceGroup) {
	...
	{
	    $resourceGroup = Write-Host "? LUIS Service Resource Group (existing service in Azure required)"
	}
}
...
if (-not $luisAccountRegion) {
	$luisAccountRegion = Write-Host "? LUIS Service Location"
}
...
foreach ($language in $languageArr)
{
	...
	$langCodeUnderScore = $langCode
	$langCodeUnderScore = $langCodeUnderScore -replace '\-', '_'
	...
	if ($useDispatch) {
		...
		$logsyncmessage1 = "Initializing $($langCode) dispatch model ... $dispatchName "
        $logsyncmessage1 | Out-File -Append -FilePath $logfile
		$dispatchName = "$($name)_$($langCodeUnderScore)_Dispatch"
		...
		$logsyncmessage = "dispatch init done.. "
        $logsyncmessage | Out-File -Append -FilePath $logfile
	...
	if ($luisFiles) {
		...
		foreach ($lu in $luisFiles)
		{
			...
			$logsyncmessage2 = "> Setting LUIS subscription key ...token: "
			$logsyncmessage2 | Out-File -Append -FilePath $logfile

			$azAccount.accessToken | Out-File -Append -FilePath $logfile
			$azAccessToken.id | Out-File -Append -FilePath $logfile

			if ($luisApp) {
				# Setting subscription key
				$addKeyResult = bf luis:application:assignazureaccount `
						--accountName $luisRtAccountName `
						--resourceGroup $resourceGroup `
						--armToken $azAccessToken.accessToken `
						--azureSubscriptionId $azAccount.id `
						--appId $luisApp.id `
						--endpoint $luisRtEndpoint `
						--subscriptionKey $luisSubscriptionKey 2>> $logFile

				if ($addKeyResult -ne "Account successfully assigned.") {
					
					$logsyncmessage4 = "> Could not assign subscription key automatically. "
					$logsyncmessage4 | Out-File -Append -FilePath $logfile
				...
				if ($useDispatch) {
					...
					$logsyncmessage3 = "> Adding $($langCode) $($lu.BaseName) app to dispatch model ..."
					$logsyncmessage3 | Out-File -Append -FilePath $logfile
					(dispatch add `
						--type "luis" `
						--name $luisApp.name `
						--id 'General'  `
	...
	if ($useQna) {
		...
		if ($qnaFiles) {		
			...
			{
				$qnacurrentSubscriptionKey = $qnaSubscriptionKey

				if($langCode -eq 'ar-ar')
				{
					$qnacurrentSubscriptionKey = $qnaArSubscriptionKey;
				}
				...
				$qnaKb = DeployKB `
                    -name $name `
                    -luFile $lu `
                    -qnaSubscriptionKey $qnacurrentSubscriptionKey `
				...
				if ($qnaKb) {
					if ($useDispatch -and -not $excludedKbFromDispatch.Contains($lu.BaseName)) {
						...
						$logsyncmessage5 = "> Adding $($langCode) $($lu.BaseName) kb to dispatch model ..."
						$logsyncmessage5 | Out-File -Append -FilePath $logfile
				  
						(dispatch add `
							--type "qna" `
							--name $lu.BaseName `
							--id $qnaKb.kbId  `
							--key $qnacurrentSubscriptionKey `
                    ...
                    $qnaEndpointKeys = bf qnamaker:endpointkeys:list `
                        ...
                        --subscriptionKey $qnacurrentSubscriptionKey  | ConvertFrom-Json
					...
					$config.knowledgebases += @{
						...
						name = $lu.BaseName
						kbId = $qnaKb.kbId
						subscriptionKey = $qnacurrentSubscriptionKey
...
$settings | ConvertTo-Json -depth 100 | Out-File -Encoding utf8 $(Join-Path $outFolder "outputCognitiveModels.json")