    ...
    foreach ($sector in $sectors.GetEnumerator()) {
      ...
      foreach ($child in $sectorCollection) {
        ...
        foreach ($entity in $entities.GetEnumerator()) {
          ...
          if ($entity.Name.ToLower().Contains($child.Url.ToLower())) {
...
function AddWebpartToPage($url, $nodeLevel) {
  try {
    ...
    if ($page -ne $null) {
      ...
      #Set-PnPPage -Identity $page -CommentsEnabled:$False
      ...
      foreach ($webpartSection in $nodeLevel.webpartSection) {
        ...
        foreach ($webpartSection in $webpartSection.webpart) {
          ...
          if ($webpartSection.DefaultWebPartType -eq 'List') {
            ...
            #Set-PnPPageWebPart -Page $page -Identity $listWebpart.InstanceId -PropertiesJson $jsonProperties
          }
