...
function Complete-Cleanup() {
	...
	$psfileunpublishcontenttypesscript = Resolve-Path $PSScriptRoot".\unpublishcontenttypes.ps1"
	if ($null -ne $loginResult) {
		.$psfileunpublishcontenttypesscript @paramslogin
	}

	Write-Host 'System will wait for 30 minutes before proceeding with cleanup' -ForegroundColor Green
