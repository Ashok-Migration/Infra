	 ...
	 foreach ($NavigationLevel1Node in $allNavigationNodeQuickLaunch) {
	  ...
	  if ($TopNav.Children) {
				...
				foreach($child in $TopNav.Children){
					...
					if($Level1Nav.Children){
						...
						foreach($childLevel2 in $Level1Nav.Children){
							foreach ($engNode in $termsXML.Descendants($child.Title.replace(' ',''))){
										$EngName = Get-AttributeValue $engNode "nodeName"
										write-Host $childLevel2.Title
										write-Host $EngName
					...
					foreach ($engNode in $termsXML.Descendants($NavigationLevel1Node.Title.replace(' ',''))) {
						$EngName = Get-AttributeValue $engNode "nodeName"
		 ...
		 foreach ($engNode in $termsXML.Descendants("EngNode")) {
				...
				$EngName = Get-AttributeValue $engNode "nodeName"
				Write-Host $EngName
				write-host $NavigationLevel1Node.Title
